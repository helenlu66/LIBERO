...
class Libero_Tabletop_Manipulation(BDDLBaseDomain):
    ...
    def _load_objects_in_arena(self, mujoco_arena):
        ...
        for category_name in objects_dict.keys():
            for object_name in objects_dict[category_name]:
                # print(f"Loading object: {object_name} in category: {category_name}")
    ...
    def _load_sites_in_arena(self, mujoco_arena):
        ...
        for object_region_name in list(region_dict.keys()):
            # print(f"Setting up region: {object_region_name}")
    ...
    def _check_success(self):
        ...
        # print(f"Goal state to evaluate: {goal_state}")
        result = True
        for state in goal_state:
            predicate_result = self._eval_predicate(state)
            # print(f"Evaluating predicate {state}: {predicate_result}")
            result = predicate_result and result
        # print(f"Final success result: {result}")
        return result
    ...
    def _eval_predicate(self, state):
        if len(state) == 3:
            ...
            # Log current states of the involved objects
            # print(f"Current state of {object_1_name}: {self.object_states_dict[object_1_name]}")
            # print(f"Current state of {object_2_name}: {self.object_states_dict[object_2_name]}")
            ...
            # print(f"Predicate: {predicate_fn_name}({object_1_name}, {object_2_name}) = {result}")
            return result
        elif len(state) == 2:
            ...
            # Log current state of the object
            # print(f"Current state of {object_name}: {self.object_states_dict[object_name]}")

            result = eval_predicate_fn(
                predicate_fn_name, self.object_states_dict[object_name]
            )
            # print(f"Predicate: {predicate_fn_name}({object_name}) = {result}")
            return result
    ...
    def _post_process(self):
        ...
        # Log the parsed goal state after the parsed_problem is initialized
        # if hasattr(self, "parsed_problem"):
            # print(f"Parsed goal state: {self.parsed_problem.get('goal_state', 'Goal state not found')}")
        # else:
            # print("parsed_problem is not initialized.")
            
        # Log object states after they are initialized
        # if hasattr(self, "object_states_dict"):
            # print(f"Initial object states: {self.object_states_dict}")
        # else:
            # print("object_states_dict is not initialized.")
